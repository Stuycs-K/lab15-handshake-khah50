...
int server_handshake_half(int *to_client, int from_client){
    ...
    if(read(from_client, client_pipe, HANDSHAKE_BUFFER_SIZE) <= 0){
        perror("failed read pipe");
        close(from_client);
        return -1;
    }
    printf("[subserver] Received pipe: %s\n", client_pipe);
